...
function taskMetadata(body: unknown) {
  ...
  if (value.subject === null) update.subject = null;
  else if (typeof value.subject === "string")
    update.subject = value.subject.trim().slice(0, 50) || null;
  if (
  ...
  if (Number.isInteger(value.difficulty))
...
async function getTaskWithCounts(id: number, userId: string) {
  ...
  const items = await db
    ...
    .where(eq(checklistItemsTable.taskId, id));
  return {
    ...task,
...
function withChecklistCounts<T extends { id: number }>(
  tasks: T[],
  items: Array<{ taskId: number; completed: boolean }>,
) {
...
router.post("/tasks", async (req, res): Promise<void> => {
  ...
  if (parsed.data.projectId && !metadata.subject) {
    const [project] = await db
      .select({ subject: projectsTable.subject })
      .from(projectsTable)
      .where(
        and(
  ...
  const [task] = await db
    .insert(tasksTable)
    .values({ ...parsed.data, ...metadata, vpValue, userId })
    .returning();
...
router.patch("/tasks/bulk-reschedule", async (req, res): Promise<void> => {
  ...
  const selected = await db
    .select({ externalSource: tasksTable.externalSource })
    .from(tasksTable)
    .where(and(inArray(tasksTable.id, taskIds), eq(tasksTable.userId, userId)));
  if (selected.some((task) => task.externalSource)) {
    res
      .status(409)
      .json({
        error:
          "Canvas controls imported due dates. Remove Canvas tasks from this bulk action.",
      });
    return;
  }
...
router.patch("/tasks/:id", async (req, res): Promise<void> => {
  ...
  const [existing] = await db
    ...
    .from(tasksTable)
    .where(
      and(eq(tasksTable.id, params.data.id), eq(tasksTable.userId, userId)),
    );
  if (!existing) {
    res.status(404).json({ error: "Task not found" });
    return;
  ...
  const protectedFields = [
    ...
    "startDate",
    "subject",
    ...(existing.externalSource === "canvas" ? ["status"] : []),
  ];
  if (
    ...
    res
      ...
      .json({
        error:
          "Canvas controls this task's title, course, due time, and submission state.",
      });
    return;
  }
...
router.delete("/tasks/:id", async (req, res): Promise<void> => {
  ...
  if (existing?.externalSource) {
    res
      ...
      .json({
        error:
          "Use Remove from Nimbus for Canvas tasks so they remain ignored on future syncs.",
      });
    return;
  }
...
router.post("/tasks/:id/complete", async (req, res): Promise<void> => {
  ...
  if (existing?.externalSource === "canvas") {
    res
      .status(409)
      .json({ error: "Canvas controls submission completion for this task." });
    return;
  }
