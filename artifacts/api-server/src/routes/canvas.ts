...
function configuredBaseUrl() {
  return validateCanvasUrl(
    process.env.CANVAS_BASE_URL ?? "https://fisd.instructure.com",
    "base",
  );
}
function callbackUrl(req: Parameters<Parameters<typeof router.get>[1]>[0]) {
  return (
    process.env.CANVAS_REDIRECT_URI ??
    `${req.protocol}://${req.get("host")}/api/canvas/oauth/callback`
  );
}
async function integrationFor(userId: string) {
  return (
    await db
      ...
      .where(
        and(
          eq(externalIntegrationsTable.userId, userId),
          eq(externalIntegrationsTable.provider, "canvas"),
        ),
      )
  )[0];
}
...
router.get("/canvas/status", async (req, res): Promise<void> => {
  ...
  const [courses, latestRun, suggestions, ignored] = await Promise.all([
    db
      .select({
        id: externalCoursesTable.id,
        externalCourseId: externalCoursesTable.externalCourseId,
        name: externalCoursesTable.name,
        courseCode: externalCoursesTable.courseCode,
        subjectId: externalCoursesTable.subjectId,
        enabled: externalCoursesTable.enabled,
      })
    ...
    db
      ...
      .where(eq(integrationSyncRunsTable.integrationId, integration.id))
      .orderBy(desc(integrationSyncRunsTable.createdAt))
      .limit(1)
    ...
    db
      ...
      .where(
        and(
          eq(projectSuggestionsTable.integrationId, integration.id),
          eq(projectSuggestionsTable.status, "pending"),
        ),
      ),
    db
      .select()
  ...
  res.json({
    ...
    needsCourseSelection:
      integration.mode === "oauth" &&
      courses.some((course) => !course.subjectId),
  });
});
...
router.get("/canvas/oauth/start", async (req, res): Promise<void> => {
  ...
  if (!clientId || !process.env.CANVAS_CLIENT_SECRET) {
    res.status(503).json({
      error:
        "Canvas OAuth is not configured yet. Use the calendar feed fallback.",
    });
    return;
  }
...
router.get("/canvas/oauth/callback", async (req, res): Promise<void> => {
  ...
  try {
    ...
    try {
      response = await fetch(new URL("/login/oauth2/token", baseUrl), {
        method: "POST",
    ...
    if (!token.access_token)
      throw new Error("Canvas did not return an access token");
    const now = new Date();
    const [integration] = await db
      .insert(externalIntegrationsTable)
      .values({
        userId,
        provider: "canvas",
        mode: "oauth",
        ...
        tokenExpiresAt: token.expires_in
          ...
          : null,
        externalUserId: token.user?.id ? String(token.user.id) : null,
        status: "connected",
        updatedAt: now,
      })
      .onConflictDoUpdate({
        target: [
          externalIntegrationsTable.userId,
          externalIntegrationsTable.provider,
        ],
        set: {
          mode: "oauth",
          baseUrl,
    ...
    res
      .status(502)
      .send(`Canvas connection failed: ${redactIntegrationError(error)}`);
  }
});
...
router.post("/canvas/feed/connect", async (req, res): Promise<void> => {
  ...
  if (!userId) return;
  try {
    const feedUrl = validateCanvasUrl(String(req.body?.feedUrl ?? ""), "feed");
    const now = new Date();
    const [integration] = await db
      .insert(externalIntegrationsTable)
      .values({
        ...
        feedUrlEncrypted: encryptIntegrationSecret(feedUrl),
        status: "connected",
        updatedAt: now,
      })
      .onConflictDoUpdate({
    ...
    res.status(400).json({ error: redactIntegrationError(error) });
  }
});
...
router.put("/canvas/courses", async (req, res): Promise<void> => {
  ...
  if (!userId) return;
  const integration = await integrationFor(userId);
  if (!integration || integration.mode !== "oauth") {
    res.status(404).json({ error: "Canvas OAuth connection not found" });
    return;
  }
  const selections = Array.isArray(req.body?.courses)
    ? (req.body.courses as Array<{
        id: number;
        enabled?: boolean;
  ...
  for (const course of courses) {
    const selection = selections.find((item) => Number(item.id) === course.id);
    ...
    if (selection.enabled !== false && !subjectId) {
      ...
      let subject = current.find(
        (item) =>
          item.name.toLowerCase() === course.name.toLowerCase() &&
          !claimedSubjectIds.has(item.id),
      );
      if (!subject) {
        const base = course.name.slice(0, 32);
        const suffix = course.courseCode ?? course.externalCourseId;
        [subject] = await db
...
router.post("/canvas/sync", async (req, res): Promise<void> => {
  ...
  const integration = await integrationFor(userId);
  if (!integration) {
    res.status(404).json({ error: "Connect Canvas first" });
    return;
  }
  const [running] = await db
    ...
    .where(
      and(
        eq(integrationSyncRunsTable.integrationId, integration.id),
        inArray(integrationSyncRunsTable.status, ["queued", "running"]),
      ),
    )
    .orderBy(desc(integrationSyncRunsTable.createdAt))
    .limit(1);
  if (running) {
    if (!activeRuns.has(integration.id)) {
      activeRuns.add(integration.id);
      void runCanvasSync(running.id, integration.id)
        .catch(() => undefined)
        .finally(() => activeRuns.delete(integration.id));
    }
    res.status(202).json(running);
    return;
  }
  const [run] = await db
    .insert(integrationSyncRunsTable)
    .values({ integrationId: integration.id, status: "queued" })
    .returning();
  if (!activeRuns.has(integration.id)) {
    activeRuns.add(integration.id);
    void runCanvasSync(run.id, integration.id)
      .catch(() => undefined)
      .finally(() => activeRuns.delete(integration.id));
  }
...
router.get("/canvas/sync/:id", async (req, res): Promise<void> => {
  ...
  const [run] = await db
    ...
    .where(
      and(
        eq(integrationSyncRunsTable.id, Number(req.params.id)),
        eq(integrationSyncRunsTable.integrationId, integration.id),
      ),
    );
  if (!run) {
...
router.get("/canvas/events", async (req, res): Promise<void> => {
  ...
  res.json(
    categories?.size
      ? events.filter((event) => categories.has(event.category))
      : events,
  );
});
...
router.get("/canvas/ignored", async (req, res): Promise<void> => {
  ...
  const items = await db
    ...
    .from(externalSyncIgnoresTable)
    .where(eq(externalSyncIgnoresTable.integrationId, integration.id))
    .orderBy(desc(externalSyncIgnoresTable.createdAt));
    ...
    query
      ? items.filter(
          (item) =>
            ...
            item.externalId.includes(query),
        )
      : items,
  );
});
...
router.post("/canvas/ignore", async (req, res): Promise<void> => {
  ...
  const externalType =
    req.body?.externalType === "event" ? "event" : "assignment";
  let ids = Array.isArray(req.body?.externalIds)
    ? req.body.externalIds.map(String).slice(0, 500)
    : [String(req.body?.externalId ?? "")];
  if (externalType === "event" && allowedCategories.has(req.body?.category)) {
    ids = (
      await db
        .select({ id: externalCalendarEventsTable.externalEventId })
        .from(externalCalendarEventsTable)
        .where(
          and(
            eq(externalCalendarEventsTable.integrationId, integration.id),
            eq(externalCalendarEventsTable.category, req.body.category),
          ),
        )
    ).map((item) => item.id);
  }
  if (
    ...
    typeof req.body?.externalCourseId === "string"
  ) {
    ids = (
      await db
        .select({ id: tasksTable.externalId })
        .from(tasksTable)
        .where(
          and(
            eq(tasksTable.externalIntegrationId, integration.id),
            eq(tasksTable.externalCourseId, req.body.externalCourseId),
          ),
        )
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(externalCoursesTable.externalCourseId, req.body.externalCourseId),
        ),
      );
  }
  for (const externalId of ids.filter(Boolean)) {
    let title: string | null = null;
    if (externalType === "assignment") {
      const [task] = await db
        .select()
        .from(tasksTable)
        .where(
      ...
      const [event] = await db
        ...
        .where(
          and(
            eq(externalCalendarEventsTable.userId, userId),
            eq(externalCalendarEventsTable.integrationId, integration.id),
            eq(externalCalendarEventsTable.externalEventId, externalId),
          ),
        );
    ...
    await db
      .insert(externalSyncIgnoresTable)
      .values({
...
router.post("/canvas/ignored/:id/restore", async (req, res): Promise<void> => {
  ...
  const [item] = await db
    ...
    .where(
      and(
        eq(externalSyncIgnoresTable.id, Number(req.params.id)),
        eq(externalSyncIgnoresTable.integrationId, integration.id),
      ),
    )
    .returning();
  if (!item) {
    res.status(404).json({ error: "Ignored item not found" });
    return;
  }
    ...
    await db
      ...
      .where(
        and(
          eq(tasksTable.userId, userId),
          eq(tasksTable.externalIntegrationId, integration.id),
          eq(tasksTable.externalId, item.externalId),
        ),
      );
  else {
    await db
      ...
      .where(
        and(
          eq(externalCalendarEventsTable.userId, userId),
          eq(externalCalendarEventsTable.integrationId, integration.id),
          eq(externalCalendarEventsTable.externalEventId, item.externalId),
        ),
      );
    await db
      .update(tasksTable)
...
router.post("/canvas/ignored/restore-all", async (req, res): Promise<void> => {
  const userId = requireUser(req, res);
  if (!userId) return;
  const integration = await integrationFor(userId);
  if (!integration) {
...
router.delete("/canvas/ignored/:id", async (req, res): Promise<void> => {
  ...
  if (item.externalType === "assignment")
    ...
    await db
      .delete(externalCalendarEventsTable)
      .where(
        and(
          eq(externalCalendarEventsTable.userId, userId),
          eq(externalCalendarEventsTable.integrationId, integration.id),
  ...
  async (req, res): Promise<void> => {
    ...
    res.json(
      tasks.map((task) => ({
        ...
        suggestion: suggestCanvasSubject(task.title, subjects),
      })),
    );
  },
);
...
router.post(
  "/canvas/organization-suggestions/apply",
  async (req, res): Promise<void> => {
    const userId = requireUser(req, res);
    if (!userId) return;
    const integration = await integrationFor(userId);
    ...
    const requested = Array.isArray(req.body?.suggestions)
      ? (req.body.suggestions.slice(0, 500) as Array<{
          taskId?: unknown;
          subjectId?: unknown;
        }>)
      : [];
    ...
    for (const item of requested) {
      ...
      const subject = subjectById.get(Number(item.subjectId));
      if (!Number.isInteger(taskId) || !subject) continue;
      const [task] = await db
        .update(tasksTable)
        .set({ subject: subject.name, organized: true })
        .where(
          and(
...
router.delete("/canvas/items", async (req, res): Promise<void> => {
  const userId = requireUser(req, res);
  if (!userId) return;
  const integration = await integrationFor(userId);
  if (!integration) {
  ...
  for (const task of tasks)
    if (task.externalId) {
      ...
      ignores.set(`${externalType}:${task.externalId}`, {
        integrationId: integration.id,
        externalType,
        externalId: task.externalId,
        title: task.title,
        reason: "Removed all Canvas items",
      });
    }
  for (const event of events)
    ignores.set(`event:${event.externalEventId}`, {
      integrationId: integration.id,
      externalType: "event",
  ...
  res.json({
    removedTasks: tasks.length,
    removedEvents: events.length,
    ignoredItems: ignores.size,
  });
});
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [suggestion] = await db
      ...
      .where(
        and(
          eq(projectSuggestionsTable.id, Number(req.params.id)),
          eq(projectSuggestionsTable.integrationId, integration.id),
          eq(projectSuggestionsTable.status, "pending"),
        ),
      );
    ...
    const tasks = await db
      ...
      .where(
        and(
          eq(tasksTable.userId, userId),
          eq(tasksTable.externalCourseId, suggestion.externalCourseId),
          inArray(tasksTable.externalId, suggestion.externalTaskIds),
        ),
      );
    if (!tasks.length) {
      res.status(409).json({
        error:
          "The linked Canvas tasks are no longer available. Sync Canvas and try again.",
      });
      return;
    }
    const course = (
    ...
    const subject = course?.subjectId
      ? (
          await db
            .select()
            .from(subjectsTable)
            .where(eq(subjectsTable.id, course.subjectId))
        )[0]
      : null;
    const dueDate =
      tasks
    ...
    const project = await db.transaction(async (tx) => {
      const [created] = await tx
        ...
        .values({
          ...
          subject: subject?.name ?? tasks[0]?.subject ?? null,
          dueDate,
          priority: "medium",
          status: "active",
          color: subject?.color ?? "#0f6cbf",
        })
        .returning();
      await tx
        .update(tasksTable)
      ...
      await tx
        ...
        .where(eq(projectSuggestionsTable.id, suggestion.id));
      return created;
    });
...
router.delete("/canvas", async (req, res): Promise<void> => {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .delete(externalIntegrationsTable)
