...
type RewardsResponse = { balance: number; owned: string[]; equipped: { avatar: string; frame: string; pet: string }; avatarStyle: string; items: Reward[] };

function AvatarPreview({ style, name, large = false }: { style: string; name: string; large?: boolean }) {
  const palette: Record<string, string> = { bolt: 'bg-primary text-primary-foreground', orbit: 'bg-violet-500 text-white ring-4 ring-violet-300/40', signal: 'bg-cyan-500 text-slate-950 ring-4 ring-cyan-200/50', ember: 'bg-orange-500 text-slate-950 ring-4 ring-orange-200/50', prism: 'bg-fuchsia-500 text-white ring-4 ring-fuchsia-200/50', mono: 'bg-zinc-900 text-white ring-4 ring-zinc-400/30', aurora: 'bg-emerald-400 text-slate-950 ring-4 ring-fuchsia-300/40' };
...
export default function Profile() {
  ...
  return <div className="space-y-5">
    <section className="bento-card p-6 sm:p-8"><div className="flex items-center gap-4"><div className={`relative rounded-[1.7rem] p-1 ${rewards?.equipped.frame !== 'none' ? 'ring-4 ring-primary/35 shadow-[0_0_28px_hsl(var(--primary)/.25)]' : ''}`}><AvatarPreview style={rewards?.avatarStyle ?? 'bolt'} name={name} large />{rewards?.equipped.pet !== 'none' && <motion.div animate={{ y: [0,-4,0], rotate: [-2,2,-2] }} transition={{ duration: 2.4, repeat: Infinity }} className="absolute -bottom-2 -right-4 flex h-9 w-9 items-center justify-center rounded-xl border-2 border-background bg-secondary text-sm font-black text-secondary-foreground shadow-lg">{rewards?.equipped.pet === 'pixel-spark' ? '✦' : rewards?.equipped.pet === 'cloud-bit' ? '☁' : '◆'}</motion.div>}</div><div><p className="text-xs font-black uppercase text-primary">Velocity profile</p><h1 className="tech-title mt-1 text-3xl">{name}</h1><p className="mt-1 text-sm text-muted-foreground">Build your setup. Keep your momentum.</p></div></div></section>
    ...
    <section className="bento-card p-5"><div className="flex items-center gap-2"><ShoppingBag className="h-5 w-5 text-primary" /><h2 className="tech-title text-lg">Customization collection</h2></div><p className="mt-1 text-sm text-muted-foreground">Combine one avatar core, one frame, and one subtle companion.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{rewards?.items.map((item) => { const owned = rewards.owned.includes(item.id); const equipped = rewards.equipped[item.kind as keyof RewardsResponse['equipped']] === item.id; return <div key={item.id} className={`rounded-xl border p-4 transition-all ${equipped ? 'border-primary bg-primary/10 shadow-[0_0_22px_hsl(var(--primary)/.16)]' : 'border-border bg-muted/20'}`}><div className="flex h-12 items-center justify-between">{item.kind === 'avatar' ? <AvatarPreview style={item.style} name={item.name} /> : <span className={`flex h-11 w-11 items-center justify-center rounded-xl bg-secondary/15 text-xl text-secondary ${item.kind === 'frame' ? 'ring-2 ring-primary/40' : ''}`}>{item.kind === 'pet' ? item.style === 'spark' ? '✦' : item.style === 'cloud' ? '☁' : '◆' : '◇'}</span>}<Sparkles className="h-4 w-4 text-secondary" /></div><p className="mt-4 font-black">{item.name}</p><p className="mt-1 text-xs uppercase text-muted-foreground">{item.kind}</p>{owned ? <button disabled={working === item.id || equipped} onClick={() => equip(item)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2 text-xs font-black text-primary-foreground disabled:opacity-55">{equipped ? <><Check className="h-3.5 w-3.5" /> Equipped</> : 'Equip'}</button> : <button disabled={working === item.id} onClick={() => purchase(item)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-secondary px-3 py-2 text-xs font-black text-secondary-foreground disabled:opacity-55"><Lock className="h-3.5 w-3.5" /> {item.cost} VP</button>}</div>; })}</div></section>
